...
if (isset($_POST['action'])) {
    ...
    switch ($action) {
        case 'add_program':
            $stmt = $conn->prepare("INSERT INTO programs (name, picture, description, details) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("ssss", $_POST['name'], $_POST['picture'], $_POST['description'], $_POST['details']);
            $stmt->execute();
            break;
        case 'add_animal':
            $stmt = $conn->prepare("INSERT INTO animals (name, picture, description, details) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("ssss", $_POST['name'], $_POST['picture'], $_POST['description'], $_POST['details']);
            $stmt->execute();
            break;
        case 'update_program':
...
<html>
<head>
    <title>ZooParc - Member Dashboard</title>
...
.navbar {
    ...
    top: 0px; /* Adjust this value to the height of the header if needed */
    left: 0;
...
@media (min-width: 481px) and (max-width: 768px) {
    ...
    img {
        width: 100px; /* Adjust image size for medium screens */
    }
}
...
<div class="navbar">
        <a href="#" class="logo">Member Dashboard</a>
        <div class="links">
...
<section class="adding">
    ...
    <form action="..\php\member_dashboard.php" method="post">
